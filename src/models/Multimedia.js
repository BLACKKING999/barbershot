...
class Multimedia {
  ...
  static async buscar(termino, limite = 20) {
    ...
    try {
      const [rows] = await pool.execute(query, [busquedaParam, busquedaParam, busquedaParam, busquedaParam, limite]);
      return rows;
    } catch (error) {
  ...
  static async obtenerPorFormato(formato, opciones = {}) {
    ...
    try {
      const [rows] = await pool.execute(query, [formato, activo, limite]);
      return rows;
    } catch (error) {
  ...
  static async obtenerPorAutor(autor, opciones = {}) {
    ...
    try {
      const [rows] = await pool.execute(query, [autor, activo, limite]);
      return rows;
    } catch (error) {
  ...
  static async obtenerPorPalabraClave(palabraClave, opciones = {}) {
    ...
    try {
      const [rows] = await pool.execute(query, [activo, palabraClaveParam, limite]);
      return rows;
    } catch (error) {
  ...
  static async marcarDestacado(id, destacado) {
    ...
    try {
      const [result] = await pool.execute(query, [destacado ? 1 : 0, id]);
